...
| 機能 | 状態 |
|---|---|
| 冷蔵庫・常備品の管理(一覧/追加/削除) | ✅ backend/frontend とも実装済み。冷蔵庫は入庫日をその場で編集可能(ブラウザ操作で確認済み) |
| 既存データ(`ai-project/menu`)の移行スクリプト | ✅ 実装済み、実データで動作確認済み |
| 献立提案(冷蔵庫・履歴を踏まえてClaude APIが献立全体+調理タイムラインを提案) | ✅ backend/frontend実装済み。実際のANTHROPIC_API_KEYでの成功パスも確認済み。未来日付を指定した提案にも対応(ブラウザ操作で確認済み) |
| 献立記録・カレンダー閲覧 | ✅ backend/frontend とも実装済み。月表示グリッド+朝昼晩の色分けドット+日付選択でその日の献立を表示。LLM提案を経由しない手動登録にも対応(ブラウザ操作で確認済み) |
...
| レシピ管理(一覧/詳細/追加/編集/削除、既存Markdownからの移行) | ✅ backend/frontend とも実装済み。ブラウザ操作で動作確認済み |
| API利用料の可視化 | ✅ サーバーログ+画面フッター(今回/累計)実装済み |
| 献立設定(人数・被り回避日数を個人設定のデフォルト値として管理) | ✅ backend/frontend とも実装済み(TestClient・ブラウザ操作で動作確認済み) |
| メニューのジャンル分類(カレー/丼など、料理名から自動判定) | ✅ backend実装済み(TestClientで動作確認済み)。被り回避ロジックへの活用は未着手(#24) |
| メニューの使用食材の参照(分量なし) | ✅ backend/frontend とも実装済み。献立提案が返す各品目の食材名を記録・表示(TestClientで動作確認済み) |
| 作り置きフラグ→冷蔵庫への自動登録 | ✅ backend/frontend とも実装済み。献立提案で「作り置き」にチェックした品目は記録時に冷蔵庫(category=作り置き料理)へ自動追加される(TestClientで動作確認済み) |
