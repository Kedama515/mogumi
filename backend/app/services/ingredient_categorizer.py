"""食材名からカテゴリ(野菜/肉/魚介 など)を解決する。

`ingredient_categories`(正規化名+カテゴリ)と`ingredient_aliases`(表記揺れ
→正規化食材)の2テーブルをマスターとして使い、全household共通でキャッシュする。

1. まずSEED_INGREDIENTSで一般的な食材+よくある表記揺れを起動時に登録しておく
   (LLM不要)
2. 未知の表記だけ、Claude(Haiku)に正規化名+カテゴリを1回だけ判定させ、
   結果をマスターに保存する(次回以降は誰の冷蔵庫でも即ヒットする)
"""

from sqlalchemy.orm import Session

from .. import models
from .claude_client import classify_ingredient

CATEGORY_VOCAB = ["野菜", "肉", "魚介", "卵・乳製品", "主食", "果物", "その他"]
