...
| **献立**(中央・円形強調) | フォーム(食事は常時表示、人数・直近日数は普段は個人設定を使い、折りたたみを開けば今回だけ上書き可能)を送って献立全体の提案(各品目の使用食材含む)を受け取り、気に入ったら記録する | `GET /api/settings`, `POST /api/suggestions`, `POST /api/meals` |
...
設定はホームタブに含まれるため下部タブには出てこない(`tab==='settings'`のとき、ホームがアクティブ表示になる)。設定画面は「個人設定」(人数・被り回避の参照日数、`GET/PUT /api/settings`)と「アカウント」(ログアウト)にセクション分けしている。ログインユーザーの作成は`backend`側の`scripts/create_user.py`から行う(このアプリに登録画面はない)。
